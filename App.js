...
// useState para usar estados no app
import { StyleSheet, Text, View, Picker, TouchableOpacity } from 'react-native';
//Picker -> abre uma janela de opções, é diferente da modal;
//TouchableOpacity é um tipo diferente de botão, o texto fica como botão
import { LinearGradient } from 'expo-linear-gradient';
//tem que instalar isso aqui no projeto, o comando é: expo install expo-linear-gradient
//é uma cor sólida...
...
export default function App() {
  ...
  //estado que controla o app
  const [estado,setarEstado] = useState('selecionar');

  //estado inicial dos segundos
  const [segundos,setarSegundos] = useState(1);

  //estado inicial dos minutos
  const [minutos,setarMinutos] = useState(0);

  //estado inicial dos alarmes, utilizando vetores
  const [alarmeSound,setarAlarmeSound] = useState([
    {
      id:1,
      selecionado: true,
      som:'alarme 1',
      file: require('./assets/alarme1.mp3')//usar o som
    },
    {
      id:2,
      selecionado: false,
  ...
  //vetor para salvar números
  var numeros = [];
  for(var i = 1; i<=60; i++){
    numeros.push(i); //metodo para adicionar dados ao vetor
  }
  ...
  if(estado == 'selecionar'){
    return (
      <View style={styles.container}>
        ...
        <LinearGradient
          // Background Linear Gradient é a cor de funda sólida
...
//COMANDOS
//<View style={{flexDirection:'row'}}> deixa os itens alinhados na mesma direção (horizontal)
...
const styles = StyleSheet.create({
  ...
  btnIniciar:{
    ...
    borderRadius:50,
    marginTop:30,
    borderColor:'white', //deixa uma borda bonita
    borderWidth:2
  },
  container: {
    flex: 1,
    //backgroundColor: 'rgb(80, 50, 168)',
    // para pegar mais cores, google pick
    alignItems: 'center',
